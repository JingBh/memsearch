...
#
# ZCode runs Claude Code-compatible plugin hooks: the same stdin JSON, the same
# ${CLAUDE_PROJECT_DIR}/${ZCODE_PROJECT_DIR} variables, and the same
# hooks/hooks.json layout. This file mirrors the Claude Code plugin's common.sh
# so the two stay easy to diff.

set -euo pipefail

# Capture and maintenance workers spawned by these hooks set MEMSEARCH_DISABLE so
# that nothing they launch can re-enter the hooks and write nested journal entries.
...
# Use timeout to prevent indefinite blocking where the stdin pipe may not close properly.
...
# Memory directory and memsearch state directory are project-scoped. ZCode passes
# the workspace directory as ZCODE_PROJECT_DIR (and CLAUDE_PROJECT_DIR for
# compatibility); it must win over the hook process cwd, which may belong to an
# unrelated repository. Resolve a subdirectory to its own git root.
if [ -n "${ZCODE_PROJECT_DIR:-}" ] && [ -d "$ZCODE_PROJECT_DIR" ]; then
  _PROJECT_DIR="$ZCODE_PROJECT_DIR"
...
#
# Without this cleanup, rapid session open/close cycles (e.g. when ZCode
...
kill_orphaned_index() {
  # Skip in capture/maintenance workers to avoid killing the current parent's work
...
stop_watch() {
  # Skip watch management in capture/maintenance workers
  if [ "${MEMSEARCH_NO_WATCH:-}" = "1" ]; then
    return 0
  fi
