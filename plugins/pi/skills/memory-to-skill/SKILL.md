...
Pi's built-in skills system.

Resolve the centralized store before stateful MemSearch commands:

```bash
PLUGIN=$(realpath "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/extensions/memsearch")
STORE=$(bash "$PLUGIN/scripts/resolve-store.sh")
```

Stages: **0** memory journals → **1** candidate (`$STORE/skill-candidates/`,
...
```bash
printf '%s' "## <title>\n\n1. ...\n2. ..." | MEMSEARCH_DIR="$STORE" memsearch skills add \
  --name "<short-slug>" \
...
```bash
MEMSEARCH_DIR="$STORE" memsearch skills status          # pending candidate versions needing install
MEMSEARCH_DIR="$STORE" memsearch skills list            # add -j for sources / installed paths
git -C "$STORE/skill-candidates" log --oneline -5 2>/dev/null || true
```
...
MEMSEARCH_DIR="$STORE" memsearch skills install <name> --path <configured-or-user-approved-path>
```
After installation, remind the user to run Pi's `/reload` command so the newly installed skill is loaded.
...
journals yourself — they live in `$STORE/memory/*.md` — and look for
...
**Drill into the original before drafting.** The journal bullets are a lossy summary; the exact commands, flags, and paths live in the original transcript. Pi entries use `session:`, `turn:`, `leaf:`, and `transcript:` anchors. Call `memory_transcript` with those values to retrieve the selected branch **with tool calls and outputs**. Write the skill from that. If the excerpt feels incomplete, increase its context or inspect nearby turns before committing to exact commands or paths. If you cannot confirm a detail, keep the step general or omit it — never fabricate.
...
config instead. These global settings still operate on the current project's centralized store.
...
Pi reads both `.pi/skills/` and `.agents/skills/` for projects, plus
`~/.pi/agent/skills/` and `~/.agents/skills/` globally. Prefer `.agents/skills/`
when the generated skill should also be portable to other compatible agents.
...
- Never enable the feature, change install paths, or install a candidate without
  the user's go-ahead.
- Do not hand-edit the store; create candidates with `MEMSEARCH_DIR="$STORE" memsearch skills add` and let
  the git-tracked store at `$STORE/skill-candidates/` keep history.
